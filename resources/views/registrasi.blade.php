                ...
                <form action="{{ route('registrasi.store') }}" method="POST" enctype="multipart/form-data" 
                    ...
                    <div class="col-span-2">
                        <div class="flex flex-col space-y-1">
                            <!-- Tombol Upload -->
                            <label for="dokumen_pdf" 
                                class="w-full cursor-pointer bg-blue-600 text-white px-4 py-2 rounded-lg text-xs font-medium shadow hover:bg-blue-700 transition text-center">
                                Pilih Dokumen PDF
                            </label>

                            <!-- Info File -->
                            <span id="file-info" class="text-gray-500 text-xs italic text-center">
                                Belum ada file dipilih (wajib berupa PDF berisi foto & nilai)
                            </span>
                        </div>
                    ...
                    <script>
                        document.addEventListener("DOMContentLoaded", () => {
                            const inputFile = document.getElementById('dokumen_pdf');
                            const info = document.getElementById('file-info');
                            
                            inputFile.addEventListener('change', function () {
                                if (this.files && this.files[0]) {
                                    info.textContent = "📄 " + this.files[0].name;
                                    info.classList.remove("text-gray-500", "italic");
                                    info.classList.add("text-green-600", "font-semibold");
                                } else {
                                    info.textContent = "Belum ada file dipilih (wajib berupa PDF berisi foto & nilai)";
                                    info.classList.add("text-gray-500", "italic");
